...
with open('./files/extra_styles.css') as f:
    css = f.read()


st.set_page_config(layout="wide")

st.image('files/big_logo.png')
...
col_header_1, col_header_2 = st.columns(2, gap='large')
...
with col_header_2:
    st.markdown(
        "<div style='text-align:center; font-style:italic; font-size:20px; color:gray;'>💡 Empower Oakland's 2024 <a href='https://empoweroakland.com/voter-guide/' target='_blank'>Voter Guide</a> is live. Check it out for more information on the candidates and measures on the ballot.</div>",
        unsafe_allow_html=True
    )
    # st.info("💡 Empower Oakland's 2024 <a href='https://empoweroakland.com/voter-guide/' target='_blank'>Voter Guide</a> is live. Check it out for more information on the candidates and measures on the ballot.")

''

tab1, tab2, tab3 = st.tabs(["Crime Activity and Response", "Campaign Funding and Expenses", "City Services"])

with tab1:

    st.header('Crime Activity and Response')

    col_mini_filter_1, col_mini_filter_2 = st.columns(2, gap='large')
    with col_mini_filter_1:
        min_value = 2015 
...
with tab2:
    ...
    col_funding_1, col_funding_2 = st.columns([1,1], vertical_alignment="top", gap='large')
    ...
    with col_funding_2:
        ...
        df_detailed_campaign_funding = conn.query('''
                                        select Tran_Date, Filer_NamL as Filer, CONCAT(Tran_NamF,' ',Tran_NamL) as Person_on_Transaction, Tran_Emp as Person_Employer, Likely_Labor_Union, sum(Tran_Amt1) as amount 
                                        from "Campaign finance summary totals" 
                                        where Committee_Type_Name in ('Recipient Committee')
                                        group by 1,2,3,4,5
...
with tab3:
    ...
    # sort df_service_requests_aggregated first by category and then by month_date
    df_service_requests = df_service_requests.sort_values(by=['Category', 'Month_date'])

    col_cityservices_1, col_cityservices_2 = st.columns([1,1], vertical_alignment="top", gap= 'large')

    with col_cityservices_1:
    ...
    with col_cityservices_2:

        # Select a category for detailed breakdown
        service_category = st.selectbox('**Select a category for detailed breakdown below**', df_service_requests_aggregated['Category'].unique(), index=0)
